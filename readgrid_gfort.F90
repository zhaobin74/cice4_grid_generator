...
!
      ...
      if(gridout_format == 'bin') then
         nbits = storage_size(ulat)
         inquire(iolength=recl) [(i,i=1,nx_global*ny_global*nbits/8)]
         open(fid_out,file=gridout,recl=recl, &
         ...
         close(fid_out) 
         nbits = storage_size(kmti)
         inquire(iolength=recl) [(i,i=1,nx_global*ny_global*nbits/8)]
         open(fid_out,file=kmtout,recl=recl, &
                  form='unformatted',access='direct')          
